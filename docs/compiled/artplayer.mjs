  ...
  addResolution() {
    ...
    const fetchBitrate = () => {
      ...
      fetch(videoSrc, { method: "HEAD" }).then((res) => {
        ...
        if (size > 0) {
          videoBitrate = size * 8 / duration / 1e3;
        }
      }).catch(() => {
      });
    };
...
class Info extends Component {
  ...
  init() {
    ...
    const calculateFps = () => {
      ...
      const currentFrameCount = quality.totalVideoFrames || 0;
      const now = Date.now();
      const timeDiff = (now - self.lastTime) / 1e3;
      if (timeDiff >= 1) {
